...
if (!url) {
  fail("Usage: snapshot.ts <github-pr-url> [--pr-only] [--timeout-ms <milliseconds>]");
}

const result = await observeGitHubPullRequest(url, {
  timeoutMs,
  includeManagedStack: !pullRequestOnly,
});
