...
2. Use `video-tools prepare --only audio` as described in [tooling](references/tooling.md). Pass the
...
Never install tools, pull or build a container, or alter the machine.
...
- Gaps: every unread, omitted, unsupported, or failed lane.
...
- Send nothing outside the runtime.
