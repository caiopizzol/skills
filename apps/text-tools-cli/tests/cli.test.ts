...
async function runCli(args: readonly string[], cwd: string): Promise<{ exitCode: number; stdout: string; stderr: string }> {
  ...
  return new Promise((settle) => {
    execFile("bun", [CLI, ...args], { cwd, maxBuffer: 16 * 1024 * 1024 }, (error, stdout, stderr) => {
...
describe("argument parsing", () => {
  it("reads an exact path and an optional bound", () => {
    expect(parseArguments(["inspect", "notes.json", "--max-characters", "500"]))
      .toEqual({ command: "inspect", inputPath: "notes.json", maximumCharacters: 500 });
  });
  ...
  it("refuses an unknown command, a missing path, and a non-positive bound", () => {
    ...
    expect(() => parseArguments(["inspect", "a.txt", "--max-characters", "0"])).toThrow(/positive integer/);
...
describe("running from outside the monorepo", () => {
  it("inspects a complete JSON file and exits 0", async () => {
    ...
    const result = JSON.parse(run.stdout) as { outcome: string; structure: { kind: string; topLevelKeys: string[] } };
  ...
  it("exits 2 with omitted ranges when the bound truncates the file", async () => {
    ...
    const result = JSON.parse(run.stdout) as { coverage: { boundedBy: string; omittedRanges: unknown[] } };
  ...
  it("refuses a directory rather than reading it as a file", async () => {
    ...
    await expect(inspectFile({ command: "inspect", inputPath: directory })).rejects.toThrow(/not a regular file/);
  });
});
