...
import { childReferences, readmeCatalogEntries, validateCatalog, type CatalogRule } from "../src/index.ts";
...
const rules = (source: Parameters<typeof validateCatalog>[0]): CatalogRule[] =>
  validateCatalog(source).map((violation) => violation.rule).sort();

describe("a healthy catalog", () => {
  it("reports nothing when every skill is listed, owned, and described", () => {
    expect(validateCatalog(catalog([skill("read-image"), skill("read-video", "Invoke `$read-image` per frame.")]))).toEqual([]);
  });
});
...
describe("catalog violations", () => {
  ...
  it("catches a child reference this catalog does not own", () => {
    expect(rules(catalog([skill("task-context", "PDF: use `$pdf`.")]))).toEqual(["child-skill-unknown"]);
  });
  ...
  it("catches a child reference written in a reference file", () => {
    const source = catalog([skill("code-review", "", {
    ...
    expect(validateCatalog(source)).toEqual([{
      ...
      skill: "code-review",
      detail: "references/validation-lanes.md references $documents, which this catalog does not provide",
    }]);
  });
  ...
  it("accepts a reference file that names an installed sibling", () => {
    const source = catalog([
      skill("read-image"),
      skill("read-video", "", { references: { "references/sampling.md": "Each frame goes to `$read-image`." } }),
    ]);
  ...
  it.each([
    ...
    { label: "a mapping where a string belongs", agentMetadata: metadata({ text: "Use $read-image" }) },
    { label: "an empty prompt", agentMetadata: metadata("   ") },
    { label: "no interface section", agentMetadata: { document: { default_prompt: "Use $read-image" } } },
    { label: "a scalar document", agentMetadata: { document: "Use $read-image" } },
  ])("catches agent metadata with $label", ({ agentMetadata }) => {
    expect(rules(catalog([skill("read-image", "", { agentMetadata })]))).toEqual(["agent-metadata-invalid"]);
  });
  ...
  it("catches a token that appears outside the default prompt", () => {
    ...
    expect(rules(catalog([skill("read-image", "", { agentMetadata })]))).toEqual(["agent-metadata-mismatch"]);
  });

  it("catches missing agent metadata", () => {
    expect(rules(catalog([skill("read-image", "", { agentMetadata: null })]))).toEqual(["agent-metadata-missing"]);
  });

  it("catches agent metadata naming a different skill", () => {
    expect(rules(catalog([skill("read-image", "", { agentMetadata: metadata("Use $read-video on the input.") })])))
      .toEqual(["agent-metadata-mismatch"]);
  });

  it("catches a missing SKILL.md without reporting every later rule for it", () => {
    expect(rules(catalog([skill("read-image", "", { entry: null })]))).toEqual(["skill-entry-missing"]);
  });
  ...
  it("catches an unsupported frontmatter field", () => {
    const entry = "---\nname: read-image\ndescription: Reads a thing.\nversion: 2\n---\n\n# x\n";
    expect(rules(catalog([skill("read-image", "", { entry })]))).toEqual(["frontmatter-unsupported-field"]);
  });
...
describe("child reference detection", () => {
  it("reads only inline-code invocations", () => {
    expect(childReferences("Invoke `$read-image` and `$transcribe-audio`.")).toEqual(["read-image", "transcribe-audio"]);
  });
...
describe("README catalog parsing", () => {
  it("reads catalog rows with the folder each links to, ignoring unrelated repository links", () => {
    const readme = "## Skill catalog\n\n[`read-image`](skills/files/read-image/SKILL.md) and [development](docs/development.md).";
    expect(readmeCatalogEntries(readme)).toEqual([{ name: "read-image", path: "skills/files/read-image" }]);
  });
  ...
  it("accepts a row at any category depth", () => {
    expect(readmeCatalogEntries("## Skill catalog\n\n[`read-image`](skills/read-image/SKILL.md)"))
  ...
  it("ignores a link whose label and folder disagree", () => {
    expect(readmeCatalogEntries("## Skill catalog\n\n[`read-image`](skills/files/read-video/SKILL.md)")).toEqual([]);
  });
...
describe("categories and identity", () => {
  ...
  it("catches one name claimed by two folders", () => {
    ...
    expect(validateCatalog(source)).toContainEqual({
      ...
      skill: "read-image",
      detail: "provided by skills/files/read-image and skills/web/read-image; a skill name must be unique",
    });
  });
  ...
  it("catches a catalog row linking where the skill is not", () => {
    ...
    expect(validateCatalog(source)).toEqual([{
      ...
      skill: "read-image",
      detail: "the README catalog links to skills/media/read-image but the skill is at skills/files/read-image",
    }]);
  });
});
