...
const hostile: ReadonlyArray<{ label: string; code: SvgSafetyReasonCode; source: string }> = [
  ...
  },
  {
    ...
    source: '<!DOCTYPE svg [<!ENTITY leak SYSTEM "file:///etc/hostname">]>' + wrap("<text>&leak;</text>"),
  },
  {
    label: "a foreignObject element",
    code: "foreign-object",
    source: wrap('<foreignObject width="10" height="10"><div xmlns="http://www.w3.org/1999/xhtml">x</div></foreignObject>'),
  },
  {
    label: "an external stylesheet import",
    code: "external-reference",
    source: wrap('<style>@import "https://example.invalid/x.css";</style><rect width="10" height="10"/>'),
  },
  {
    label: "an external stylesheet import written as a url()",
    code: "external-reference",
    source: wrap("<style>@import url(https://example.invalid/x.css);</style><rect width=\"10\" height=\"10\"/>"),
  },
];
...
describe("inspectSvgText", () => {
  ...
  it("returns every reason rather than stopping at the first", () => {
    ...
    expect(new Set(verdict.reasons.map((reason) => reason.code))).toEqual(
      new Set(["script-element", "event-handler-attribute", "external-image", "external-reference"]),
    );
  });
  ...
  it("accepts a same-document fragment reference and an inline data image", () => {
    ...
    expect(
      inspectSvgText(wrap('<image href="data:image/png;base64,iVBORw0KGgo=" width="10" height="10"/>')).selfContained,
    ).toBe(true);
  });

  it("refuses an inline data URL whose media type is not a known raster image", () => {
    const verdict = inspectSvgText(wrap('<image href="data:text/html,%3Cscript%3Ealert(1)%3C/script%3E"/>'));
  ...
  it("flags an external url() reference such as a remote filter or pattern", () => {
    const verdict = inspectSvgText(wrap('<rect fill="url(https://example.invalid/pattern.svg#p)"/>'));
  ...
  it("refuses a base64 SVG data URL that contains an external image", () => {
    const nested = Buffer.from(wrap('<image href="https://example.invalid/x.png"/>')).toString("base64");
  ...
  it("refuses style content and XML stylesheet instructions instead of parsing CSS", () => {
    ...
    expect(inspectSvgText(`<?xml-stylesheet href="#safe"?>${wrap("<rect/>")}`).selfContained).toBe(false);
  });

  it("gives the same verdict on a second inspection of the same text", () => {
    expect(inspectSvgText(hostile[0]?.source ?? "")).toEqual(inspectSvgText(hostile[0]?.source ?? ""));
