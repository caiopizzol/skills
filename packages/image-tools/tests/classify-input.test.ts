...
const candidates: ReadonlyArray<{ label: string; path: string; bytes: Uint8Array }> = [
  ...
  {
    label: "an XML declaration then a comment",
    path: "a.png",
    bytes: encode(`<?xml version="1.0"?><!-- c -->${UNSAFE}`),
  },
  {
    label: "a DOCTYPE prefix",
    path: "a.png",
    bytes: encode(`<!DOCTYPE svg PUBLIC "" "">${UNSAFE}`),
  },
  { label: "a comment prefix", path: "a.dat", bytes: encode(`<!-- c -->${UNSAFE}`) },
  {
    label: "a root padded past any prefix window",
    path: "a.png",
    bytes: encode(`<?xml version="1.0"?>${"\n".repeat(50_000)}${UNSAFE}`),
  },
  {
    label: "whitespace padding with no declaration",
    path: "a.png",
    bytes: encode(`${" ".repeat(50_000)}${UNSAFE}`),
  },
...
describe("SVG candidate classification", () => {
  ...
  it.each([
    {
      label: "gzip magic under any name",
      path: "a.png",
      bytes: new Uint8Array(gzipSync(Buffer.from(UNSAFE))),
    },
  ...
  it("does not treat prose that merely mentions svg as a candidate", () => {
    ...
    expect(
      classifyInput("notes.txt", encode("a plain sentence about an svg file")).kind,
    ).toBe<InputKind>("unknown");
    expect(
      classifyInput("notes.txt", encode("a plain sentence with no markup at all")).kind,
    ).toBe<InputKind>("unknown");
  });

  it("reports undecodable bytes as unknown instead of scanning them as text", () => {
    expect(classifyInput("a.bin", Uint8Array.from([0x00, 0xff, 0xfe, 0x01])).kind).toBe<InputKind>(
      "unknown",
    );
  });
