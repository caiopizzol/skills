...
const manifest = JSON.parse(readFileSync(join(fixtureDirectory, "manifest.json"), "utf8")) as VideoFixtureManifest;
...
describe("video fixtures", () => {
  it("keeps the supported matrix explicit", () => {
    expect(manifest.fixtures.map(({ filename, mediaType, videoCodec, audioCodec }) => [filename, mediaType, videoCodec, audioCodec])).toEqual(
      expectedMatrix,
    );
  });
  ...
  it.each(manifest.fixtures)("records temporal, visual, and audio expectations for $filename", (fixture) => {
